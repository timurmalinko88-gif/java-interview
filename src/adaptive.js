import { loadQuestion, buildSidebarList, triggerFilterAction } from './ui.js';
import { state } from './state.js';

export function checkAdaptiveProgression() {
  if (!state.isAdaptivePlanActive) return;

  // Find next unmastered
  const nextQ = state.filteredQuestions.find(q => !state.masteredIds.includes(q.id));
  if (nextQ) {
    // Load next
    if (typeof loadQuestion === 'function') {
      const idx = state.filteredQuestions.indexOf(nextQ);
      if (idx !== -1) {
    ...
    // Plan complete!
    state.isAdaptivePlanActive = false;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.renderRoadmap = function () {
    try {
      ...
      const questions = typeof state.filteredQuestions !== 'undefined' ? state.filteredQuestions : [];
      if (!questions || questions.length === 0) {
        roadmapContent.innerHTML = '<p class="text-slate-500">Your roadmap is empty.</p>';
        return;
      }
      const mArray = typeof state.masteredIds !== 'undefined' && Array.isArray(state.masteredIds) ? state.masteredIds : [];
    ...
    applyPlanBtn.addEventListener('click', () => {
      ...
      if (typeof state.questionsList !== 'undefined') {
        ...
        if (state.filteredQuestions.length > 0) {
          state.isAdaptivePlanActive = true;
