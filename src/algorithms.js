...
export function renderAlgoList(store) {
    ...
    if (patternFilterContainer && patternFilterContainer.children.length <= 1) {
        ...
        let pillsHtml = `
            <button class="algo-pill-btn active px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap border transition-all shadow-sm bg-roast-500 text-[#2B1904] border-roast-500 hover:scale-[1.02] flex-shrink-0 flex items-center" data-pattern="all">
                <span>All Patterns</span>
                <span class="ml-2 px-1.5 py-0.5 rounded-md bg-black/15 text-[#2B1904] text-[10px] font-extrabold">${algoQuestions.length}</span>
            </button>
        `;
        ...
        patterns.forEach(p => {
            ...
            pillsHtml += `
                <button class="algo-pill-btn px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap border transition-all shadow-sm border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 bg-white dark:bg-panel-900 hover:border-roast-500 hover:text-roast-500 hover:scale-[1.02] flex-shrink-0 flex items-center" data-pattern="${p}">
                    <span>${p}</span>
                    <span class="pill-badge ml-2 px-1.5 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-extrabold">${count}</span>
                </button>
            `;
        });
