...
Несколько потоков одновременно вызывают метод, делающий `views++`. 
Что произойдет со значением счетчика при высокой нагрузке? Если `volatile` не подходит для этой задачи, то для чего он вообще нужен? Приведите пример корректного использования.

---

# Ответ

При высокой нагрузке **счетчик потеряет часть инкрементов**. 
Операция `views++` не является атомарной. Под капотом она разбивается на три шага:
1. Чтение текущего значения `views` из памяти.
...
Он идеально подходит для переменных-флагов состояния, когда один поток пишет, а другие только читают. Например, флаг остановки фонового процесса (`private volatile boolean isRunning = true;`).
