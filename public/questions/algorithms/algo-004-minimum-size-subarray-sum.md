...
1. Expand `right` pointers:
   ...
   - `right = 1 (3)` → `sum = 5`
   - `right = 2 (1)` → `sum = 6`
   - `right = 3 (2)` → `sum = 8 >= 7`! Valid window `[2, 3, 1, 2]`, length = 4.
...
3. Expand `right = 4 (4)`:
   ...
   - Shrink `left = 1 (3)` → `sum = 7 >= 7`! Valid window `[1, 2, 4]`, length = 3.
   - Shrink `left = 2 (1)` → `sum = 6 < 7`. Stop shrinking.

4. Expand `right = 5 (3)`:
   - `sum = 6 + 3 = 9 >= 7`! Valid window `[2, 4, 3]`, length = 3.
   - Shrink `left = 3 (2)` → `sum = 7 >= 7`! Valid window `[4, 3]`, **length = 2**!
